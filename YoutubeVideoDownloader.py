...
def getvideo():
    ...
    for i in streams:
        ...
        datas = str(index) + '.'.ljust(3, ' ') + str(i.quality).ljust(10, ' ') + str(
            i.extension).ljust(5, ' ') + str(i.mediatype) + ' ' + du.rjust(10, ' ') + "MB"
...
def DownloadVideoData():
    ...
    streams[downloadindex].download(
        filepath=fgr, quiet=True, callback=mycallback)
...
# Entry
urltext = StringVar()
UrlEntry = Entry(root, textvariable=urltext, font=(
    'arial', 20, 'italic bold'), width=31)
UrlEntry.place(x=220, y=150)

# Labels
IntroLabel = Label(root, text='Welcome to Youtube Video Audio Downloader ', width=36,  relief='ridge', bd=10,
                   font=('chiller', 32, 'italic bold'), fg='black')
IntroLabel.place(x=180, y=20)
...
ListBox.place(x=230, y=230)
ListBox.bind("<<ListboxSelect>>", SelectCursor)

scrollbar.configure(command=ListBox.yview)

DownloadnigSizeLabel = Label(root, text='Total Size : ', font=(
    'arial', 15, 'italic bold'), bg='olivedrab1')
DownloadnigSizeLabel.place(x=800, y=240)

DownloadnigLabel = Label(root, text='Recieved Size : ', font=(
    'arial', 15, 'italic bold'), bg='olivedrab1')
DownloadnigLabel.place(x=800, y=290)

DownloadnigTime = Label(root, text='Time Left  : ', font=(
    'arial', 15, 'italic bold'), bg='olivedrab1')
DownloadnigTime.place(x=800, y=340)

DownloadnigSizeLabelResult = Label(root, text='', font=(
    'arial', 15, 'italic bold'), bg='olivedrab1')
DownloadnigSizeLabelResult.place(x=800, y=240)

DownloadnigLabelResult = Label(root, text='', font=(
    'arial', 15, 'italic bold'), bg='olivedrab1')
DownloadnigLabelResult.place(x=650, y=290)

DownloadnigLabelTimeLeft = Label(root, text='', font=(
    'arial', 15, 'italic bold'), bg='olivedrab1')
...
DownloadingBarTextLable = Label(root, text='Downloading bar', width=36, font=('chiller', 23, 'italic bold'), fg='red',
                                bg='olivedrab1')
DownloadingBarTextLable.place(x=550, y=445)

DownloadingProgressBarLabel = Label(root, text='', width=36, font=('chiller', 40, 'italic bold'), fg='red', bg='olivedrab1',
                                    relief='raised')
DownloadingProgressBarLabel.place(x=230, y=445)


# Progressbar

DownloadnigProgressBar = Progressbar(
    DownloadingProgressBarLabel, orient=HORIZONTAL, value=0, length=100, maximum=total12)
...
DownloadButton.place(x=800, y=370)
# Create Threads
root.mainloop()
